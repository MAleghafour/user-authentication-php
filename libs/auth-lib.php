...
function sendTokenByPhon(string $phone, string|int $token)
{
    global $api;
    $receptor = $phone;
    $token2 = "";
...
function changeLoginSession(string $email = null,string $phone = null, string $session = null): bool
{
    global $pdo;
    $sql = 'UPDATE `users` SET `session` = :session WHERE `email` = :email OR `phone` = :phone;';
    $stmt = $pdo->prepare($sql);
    $stmt->execute([':session' => $session, ':email' => $email, ':phone' => $phone]);
    return $stmt->rowCount() ? true : false;
}
